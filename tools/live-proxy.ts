...
export async function startLiveProxy(options: LiveProxyOptions = {}) {
    ...
    const listen = (port: number) =>
        Bun.serve<WsData, Record<string, never>>({
            hostname: '127.0.0.1',
            port,
            idleTimeout: 0,
            async fetch(req, srv) {
    ...
    for (let port = PORT; ; port++) {
        try {
            return listen(port);
        } catch (error) {
            if (!options.autoPort || port >= 65535 || !(error instanceof Error) || !('code' in error) || error.code !== 'EADDRINUSE') throw error;
        }
    }
}

if (import.meta.main) {
    const server = await startLiveProxy({ root: process.env.B0T_INSTANCE_DIR, autoPort: process.env.B0T_AUTO_PORT === '1' });
    if (process.env.B0T_PORT_FILE) await Bun.write(process.env.B0T_PORT_FILE, String(server.port));
