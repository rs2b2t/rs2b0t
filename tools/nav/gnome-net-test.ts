...
const username = `gnet${Date.now().toString(36).slice(-7)}`;
...
try {
    ...
    await page.goto(`${base}/bot.html?GnomeCourse.obstacles=Obstacle+net&GnomeCourse.searchRadius=6`);
    await boot();
    ...
    if (!at) { fail('net tele failed'); }
    console.log(`side-on west of the pre-pipe net: (${at.x},${at.z},${at.level})\n`);

    const logsBefore = await page.evaluate(() => ((globalThis as never as R).rs2b0t.runner.ctx?.log ?? []).length);
    await page.evaluate(() => { const r = (globalThis as never as R).rs2b0t; r.runner.start(r.registry.get('GnomeCourse')); });
    console.log('started GnomeCourse (course = the one net, radius 6)\n');
    ...
    const repositioned = lines.some(l => l.includes('repositioning to its'));
    const cleared = await page.evaluate(() => {
        const b = (globalThis as never as { rs2b0t: { runner: { bot: { obstaclesCleared?: number } | null } } }).rs2b0t.runner.bot;
        return b ? (b.obstaclesCleared ?? 0) : 0;
    });
    const fin = await tile();
    console.log(`\nrepositioned=${repositioned} obstaclesCleared=${cleared} final=(${fin?.x},${fin?.z}) => ${repositioned && cleared >= 1 ? 'PASS' : 'FAIL'}`);
} finally {
    await browser.close();
}
