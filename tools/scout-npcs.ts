...
try {
    ...
    const login = async () => {
        ...
        return page.waitForFunction(() => (globalThis as never as Rs2b0t).rs2b0t.client.ingame && (globalThis as never as Rs2b0t).rs2b0t.client.sceneState === 2, undefined, { timeout: 20000 }).then(() => true).catch(() => false);
    };
    if (!(await login())) {
