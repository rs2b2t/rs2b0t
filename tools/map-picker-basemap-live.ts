...
try {
    ...
    await proof.writeSuccess(page, {
        issue: 0,
        harness: 'tools/map-picker-basemap-live.ts',
        user,
        basemap: basemapAttr,
        picked,
        status,
    ...
    console.log(
        `attach: tools/attach-live-proof-to-pr.sh --pr <n> --issue 0 --slug map-picker-basemap --harness 'bun tools/map-picker-basemap-live.ts ${base}'`
    );
} catch (e) {
    console.error(e);
