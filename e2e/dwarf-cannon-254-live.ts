...
//   HEADED=1 bun e2e/dwarf-cannon-254-live.ts --stage 4 --multi 2016 --at 2620,9797,0 --until 11 --minutes 40
...
function parse(argv: string[]): Args {
    const out: Args = {
        ...
        user: `mcan${Date.now().toString(36).slice(-6)}`,
        stage: 0,
        until: 11,
        multi: -1,
        minutes: 120,
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        else if (flag === '--multi') { out.multi = Number(value); }
...
const QUEST = 'Dwarf Cannon';
/** quest_mcannon.constant ^mcannon_complete. */
const MCANNON_COMPLETE = 11;
const FALADOR_WEST_BANK: Tile = { x: 2946, z: 3369, level: 0 };
...
/**
 * Coins, food and a melee kit. Nothing the quest hands out is seeded: the
 * railings, the toolkit, the notes and the mould all come from an NPC, and a
 * seeded copy would hide a dialogue that never fired.
 */
const BANK_SEED: BankSeedItem[] = [
    { debugName: 'coins', displayName: 'Coins', qty: 2_000_000 },
    { debugName: 'lobster', displayName: 'Lobster', qty: 40 },
    { debugName: 'rune_scimitar', displayName: 'Rune scimitar', qty: 1 },
    // Rune chainbody, never platebody: the platebody wants Dragon Slayer as well
    // as Defence 40, and the refusal is a silent false.
...
try {
    ...
    await seedItemsToBank(page, BANK_SEED, FALADOR_WEST_BANK);

    if (args.stage > 0) {
        await cheatQuiet(page, `setvar mcannon ${args.stage}`);
    }
    if (args.multi >= 0) {
        await cheatQuiet(page, `setvar mcannonmulti ${args.multi}`);
    }
    ...
    if (args.stage > 0) {
        const set = await getServerVarQuiet(page, 'mcannon');
        if (set !== args.stage) {
            fail(`setvar mcannon ${args.stage} did not take (read back ${set})`);
        }
    }
    console.log(`mcannon=${args.stage} mcannonmulti=${args.multi >= 0 ? args.multi : 'untouched'}`);

    const start = args.at ?? FALADOR_WEST_BANK;
    ...
    await page.evaluate(() => {
        ...
        g.__rs2b0t.Loadouts.save([{
            ...
            carry: [{ item: 'Lobster', qty: 8 }]
        }]);
    });
    console.log('seeded the quest loadout');

    await page.evaluate(() => sessionStorage.setItem('rs2b0t:set:AIOQuester:quests', 'mcannon'));
    await startScript(page, 'AIOQuester');
    console.log(`started AIOQuester — watching for mcannon >= ${args.until}`);
    ...
    while (Date.now() < deadline) {
        ...
        const stage = (await getServerVarQuiet(page, 'mcannon')) ?? -1;
        reached = Math.max(reached, stage);
        const t = Math.round((Date.now() - t0) / 1000);
        console.log(
            `  t=${t}s pos=${last.pos ? `${last.pos.x},${last.pos.z},${last.pos.level}` : '?'}`
            + ` mcannon=${stage} journal=${last.status} qp=${last.qp} runner=${last.runner}`
        );
        ...
        // quest-complete recolour and the QP award land a tick behind it.
        const done = args.until >= MCANNON_COMPLETE ? last.status === 'complete' : stage >= args.until;
        if (done) {
            console.log(`PASS (mcannon=${stage}, journal=${last.status}, QP=${last.qp}, ${Math.round(t / 60)}min)`);
            process.exit(0);
        }
        if (last.runner === 'stopped') {
            fail(`script stopped at mcannon=${stage} (journal=${last.status})`);
        }
        await page.waitForTimeout(10_000);
    }
    fail(`mcannon reached ${reached}, wanted ${args.until}, within ${args.minutes}min`);
} finally {
    await browser.close();
}
