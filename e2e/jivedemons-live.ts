/** Live proof for JiveDemons at the Taverley Dungeon black demons: --style --minutes --dusty --tick --no-starve.
 *  Why: combat.ts and supply.ts carry no unit tests because every function in them drives a live client, so this run is the only proof either of them holds in the pocket past the blue lair. */

// Usage: HEADED=1 bun e2e/jivedemons-live.ts [--base url] [--style mage|range] [--minutes n] [--tick ms] [--dusty] [--no-starve]
...
function parse(argv: readonly string[]): Args {
    const out: Args = {
        ...
        user: `jm${Date.now().toString(36).slice(-6)}`,
        style: 'range',
        minutes: 20,
        tickMs: 0,
        dusty: false,
...
// Why: sites.ts is not on the harness ABI, so the three area boxes and the safespots are mirrored here and a drift in either copy shows up as a failed milestone rather than a silent pass.
const BLUE: Box = { minX: 2888, maxX: 2923, minZ: 9769, maxZ: 9816, level: 0 };
const POCKET: Box = { minX: 2850, maxX: 2880, minZ: 9762, maxZ: 9797, level: 0 };
const CORRIDOR: Box = { minX: 2874, maxX: 2899, minZ: 9762, maxZ: 9775, level: 0 };
const AREA: Box[] = [BLUE, POCKET, CORRIDOR];
const SAFESPOTS: Point[] = [{ x: 2856, z: 9786, level: 0 }, { x: 2857, z: 9786, level: 0 }, { x: 2855, z: 9786, level: 0 }];
const BANK: Point = { x: 2946, z: 3369, level: 0 };

const DUSTY_ID = 1590;
const JAIL_KEY_ID = 1591;
const TARGET = 'Black Demon';
const BLUE_DRAGON = 'Blue dragon';
...
const POCKET_MS = 300_000;
const SPOT_MS = 300_000;
const KILL_MS = 600_000;
const BANK_MS = 900_000;
const SOAK_MS = 120_000;
const STARVE_WAIT_MS = 180_000;
const STARVE_BANK_MS = 900_000;
/** How long after the harness sends its own ~hit a drop of that size is the harness rather than a demon's hit. */
...
const PROOF_PATH = 'out/jivedemons-proof.json';
const SHOT_PATH = 'out/jivedemons-live.png';
...
const LOOT = ['Coins', 'Rune chainbody', 'Rune med helm', 'Blood rune', 'Law rune', 'Chaos rune', 'Herb', 'Uncut diamond', 'Uncut ruby', 'Uncut emerald', 'Uncut sapphire'];
...
// Why: it starts with no escape runes and the demon table carries no Water rune, so the first exit out of the lair is the gate walk-out assertion rather than a teleport paid for with a looted Law rune.
function kitFor(style: Style): Kit {
    const common = { foodWithdraw: PACK_FOOD, panicHp: PANIC_PCT, foodReserve: 4, healTo: 90, site: 'taverley-black-demon', teleStock: 2, bankCommonJunk: false, loot: LOOT.join(', '), logDetail: 'Verbose' };
    if (style === 'mage') {
        return {
...
const WIELDED = String(kit.settings['bow'] ?? kit.settings['staff'] ?? '');
...
function inLair(t: Point | null): boolean {
    return AREA.some(b => inBox(t, b));
}

function inPocket(t: Point | null): boolean {
    return inBox(t, POCKET);
}
...
interface BlueView { index: number; dist: number; aims: boolean }
...
interface Probe { dustyId: number; jailKeyId: number; food: string; law: string; target: string; blue: string }
...
function sample(page: Page, probe: Probe): Promise<Sample> {
    return page.evaluate(p => {
        ...
        return {
            ...
            blues: npcs.filter(n => n.name === p.blue).map(n => ({ index: n.index, dist: n.distance(), aims: n.targetsMe() })),
...
const tag = `jm${Date.now().toString(36).slice(-6)}`;
...
const blueFacing = (): { index: number; seen: number; nearest: number; offPocket: number }[] =>
    [...blueRoll.entries()].map(([index, roll]) => ({ index, ...roll }));
...
try {
    ...
    await setSettings(page, 'JiveDemons', kit.settings);
    await startScript(page, 'JiveDemons');
    console.log(`JiveDemons started; watching the key, the gate, the pocket, the safespot at ${SAFESPOTS[0].x},${SAFESPOTS[0].z}, a kill and a bank trip`);

    const probe: Probe = { dustyId: DUSTY_ID, jailKeyId: JAIL_KEY_ID, food: FOOD.name, law: 'Law rune', target: TARGET, blue: BLUE_DRAGON };
    const chain: [string, number][] = [['key', KEY_MS], ['gate', GATE_MS], ['pocket', POCKET_MS], ['safespot', SPOT_MS], ['kill', KILL_MS], ['banktrip', BANK_MS]];
    // Why: the dragons harness once passed a run of 2 kills and 0 pickups because the drops rotted inside the fight call, so a pickup reaching the pack is required here too.
    const required = ['key', 'gate', 'pocket', 'safespot', 'kill', 'banktrip', 'walkout', 'wielded', 'loot', args.dusty ? 'bankedkey' : 'coldkey', 'hpheld'];
    ...
    while (Date.now() < deadline) {
        ...
        for (const line of s.logs) {
            ...
            if (/^engaging black demon /i.test(line.msg)) { engagingLines++; }
            if (/^looted Coins$/i.test(line.msg)) { coinLoots++; }
            if (/^looted Ashes$/i.test(line.msg)) { ashLoots++; }
        ...
        if (/waiting for black demon \d+ to close/i.test(s.status)) { waitingPolls++; }
        ...
        // Why: the walk in crosses the blue lair and a three-tile passage at 2893..2896,9792..9794 an adult can stand in, and blues aggress within four tiles, so a blue facing us is recorded as the hazard the walk is expected to survive rather than asserted on.
        for (const blue of s.blues.filter(b => b.aims)) {
            const roll = blueRoll.get(blue.index) ?? { seen: 0, nearest: 99, offPocket: 0 };
            roll.seen++;
            roll.nearest = Math.min(roll.nearest, blue.dist);
            if (!inPocket(s.tile)) { roll.offPocket++; }
            if (roll.seen === 1) { console.log(`${stamp()} WARNING: ${BLUE_DRAGON} ${blue.index} is facing us at d=${blue.dist} from ${s.tile?.x},${s.tile?.z}${inPocket(s.tile) ? ' inside the pocket' : ''}`); }
            blueRoll.set(blue.index, roll);
        }
        ...
        if (inPocket(s.tile)) { mark('pocket', `inside the demon pocket at ${s.tile?.x},${s.tile?.z}`); }
        if (samePoint(s.tile, SAFESPOTS[0])) { mark('safespot', `standing on safespot 0 itself at ${SAFESPOTS[0].x},${SAFESPOTS[0].z}`); }
        ...
        if (s.kills > 0) {
            ...
            mark('kill', `${s.kills} black demon(s) down after ${engagingLines} engage line(s) and ${waitingPolls} leash poll(s)`);
        }
    ...
    const blueLine = [...blueRoll.entries()].map(([index, roll]) => `${index} nearest ${roll.nearest} over ${roll.seen} poll(s), ${roll.offPocket} of them outside the pocket`).join('; ') || 'none';
    console.log(`BLUES facing us: ${blueLine}`);
    const oneEndDrops = hpDrops.filter(d => d.eitherEnd && !d.bothEnds).length;
    console.log(`HP: ${hpDrops.length} drop(s), ${bothEndsDrops} with a safespot at both ends, ${oneEndDrops} taken walking on or off one, ${violations.length} counted as violations, ${Math.round(safespotMs / 1000)}s parked on a safespot with a demon up of ${Math.round(lairMs / 1000)}s in the lair`);
    console.log(`LOOT: ${final.looted} pickup(s), ${arrowLoots} of them Rune arrow, ${coinLoots} of them Coins, ${ashLoots} of them Ashes`);
    if (violations.length > 0) { fail(`hp fell ${violations.length} time(s) while standing on a safespot: ${JSON.stringify(violations)}`); }
    if (ashLoots > 0) { fail(`it picked up Ashes ${ashLoots} time(s) with Ashes left unticked`); }
    ...
    const proof = {
        ...
        fixture: { levels: LEVELS, pack: kit.pack, bank: bankSeed, settings: kit.settings, start: BANK, area: AREA },
        assertions: met,
        required,
