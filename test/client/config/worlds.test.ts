...
test('switches preserve mode and memory, with only approved origins and safe query fields', () => {
    ...
    expect(worldSwitchUrl(3, 'wall', from).href).toBe('https://w3.rs2b2t.com/rs2b0t/wall?lowmem=0&members=1');
...
test('every wall child stays on its wall origin and inherits its true node and memory', () => {
    const from = new URL('https://w3.rs2b2t.com/rs2b0t/wall?nodeid=10&lowmem=0&members=0&password=secret');
    for (const username of ['alice', 'bob']) {
        ...
        expect(child.searchParams.get('nodeid')).toBe('12');
