...
window.localStorage.setItem('rs2b0t:legacy:set:Fisher:fishMethod', 'Fly fishing');
window.localStorage.setItem('rs2b0t:legacy:selectedScript', 'Fisher');
window.localStorage.setItem('rs2b0t:shared:set:Miner:ore', 'Tin');
store.setItem('rs2b0t:shared:set:Miner:ore', 'Gold');
...
try {
    assert.equal(store.getItem('rs2b0t:legacy:set:Fisher:fishMethod'), 'Fly fishing');
    assert.equal(store.getItem('rs2b0t:legacy:selectedScript'), 'Fisher');
    assert.equal(store.getItem('rs2b0t:shared:set:Miner:ore'), 'Gold');
    ...
    browser.setURL('http://localhost:8081/bot.html?box=alice&world=1');
    SettingsStore.save('MarketMaker', 'blacklist', 'trade_troll');
    SettingsStore.save('Miner', 'ore', 'Coal');
    browser.setURL('http://localhost:8081/bot.html?box=alice&world=2');
    sessionStorage.clear();
    assert.equal(SettingsStore.saved('MarketMaker', 'blacklist'), 'trade_troll');
    assert.equal(SettingsStore.saved('Miner', 'ore'), 'Coal');
    SettingsStore.save('Miner', 'ore', 'Gold');
    browser.setURL('http://localhost:8081/bot.html?box=bob&world=2');
    assert.equal(SettingsStore.saved('Miner', 'ore'), undefined);
    browser.setURL('http://localhost:8081/bot.html?box=alice&world=1');
    assert.equal(SettingsStore.saved('Miner', 'ore'), 'Gold');
