    ...
    for (let lx = 0; lx < 64; lx++) {
        for (let lz = 0; lz < 64; lz++) {
            ...
            for (let dir = 0; dir < 8; dir++) {
                ...
                const dxi = DX[dir] === 1 ? 1 : 3;
                const dzi = DZ[dir] === 1 ? 0 : 2;
...
describe('PathFinder goal selection for unwalkable targets', () => {
    ...
    test('falls back to the ring when every cardinal-adjacent tile is sealed (enclave)', () => {
        ...
        expect(last.x).toBeGreaterThanOrEqual(3215);
    });
...
const WRONG_SIDE = { x: 3228, z: 3228 };
...
function houseWalls(x: number, z: number): number {
    ...
    if (inside) {
        ...
        if (z === HOUSE.z0 - 1 && x >= HOUSE.x0 && x <= HOUSE.x1) nib |= 1;
        if (x === HOUSE.x0 - 1 && z >= HOUSE.z0 && z <= HOUSE.z1) nib |= 2;
        if (z === HOUSE.z1 + 1 && x >= HOUSE.x0 && x <= HOUSE.x1) nib |= 4;
        if (x === HOUSE.x1 + 1 && z >= HOUSE.z0 && z <= HOUSE.z1) nib |= 8;
    }
    return nib;
}
