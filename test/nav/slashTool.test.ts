...
import {
    ...
    WEB_SLASH_NO_BLADE,
    WEB_SLASH_SUCCESS
} from '#/bot/nav/slashTool.js';
import { meetsRequires } from '#/bot/nav/requires.js';
import { itemsRequiredByWaypoints, missingItemsForPath } from '#/bot/nav/bankPlan.js';
...
import { agilityShortcutEdges } from '#/bot/nav/travelCatalog.js';
...
describe('elkoy maze catalog', () => {
    test('dual Elkoy portal edges require Tree Gnome Village started', async () => {
        const { elkoyMazeEdges } = await import('#/bot/nav/travelCatalog.js');
