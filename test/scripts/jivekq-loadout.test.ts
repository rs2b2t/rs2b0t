...
test('all four carry super potions and a dueling ring in a full inventory', () => {
    for (let slot = 0; slot < 4; slot++) {
        ...
        for (const id of [556, 557, 563]) expect(pack.has(id)).toBe(false);
        expect([...pack].reduce((slots, [id, count]) => slots + (id === 1854 ? 1 : count), 0)).toBe(28);
...
test('only the leader carries ropes and followers fill those slots with sharks', () => {
    ...
    expect(new Map(tripPack(0)).get(385)).toBe(16);
    for (let slot = 1; slot < 4; slot++) {
        expect(new Map(tripPack(slot)).has(954)).toBe(false);
        expect(new Map(tripPack(slot)).get(385)).toBe(18);
    }
});
