...
For issue/nav fixes, assert **game state** (tile, XP, items), not only logs. On green:
screenshot + JSON via [`tools/lib/harnessProof.ts`](../tools/lib/harnessProof.ts)
(`writeSuccess`); on red `writeFailure`. Prefer a baseline when cheap (live
