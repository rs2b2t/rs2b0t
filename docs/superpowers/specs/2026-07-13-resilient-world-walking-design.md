...
- **Backstop against a truly impossible target.** Retry-forever cannot hang a bot indefinitely: the existing `Supervisor` (10-min no-xp + no-movement, `Supervisor.ts`) → `StallGuard` restart (`StallGuard.ts`) still fires and re-anchors the script. So "never gives up" at the walker layer + "restart if wedged 10 min" at the runtime layer compose into self-healing.
