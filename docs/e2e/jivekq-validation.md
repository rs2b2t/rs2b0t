...
The team scans the visible chamber for KQ loot throughout combat and respawn waits, including items another team left behind. Rune arrows are recovered only during a recently verified death-to-respawn wait. Mithril and iron arrows are ignored. Valuable drops take priority over arrow piles, including another member's private loot claim. A local queen or fresh teammate sighting immediately cancels an arrow approach and restores protection before combat upkeep. Heartbeats elect one collector among members that can see and carry a drop; private drops remain available to their owner. Gear takes priority over other drops. Pickup requests do not block eating, and inventory growth confirms success. A vanished target releases the collector back to combat after a short confirmation grace period. Empty vials make room first; valuable gear can use the normal eating action to free a slot while preserving at least two sharks and all escape supplies. Team chat reports collection, and confirmed items appear in the Loot paint.
...
# Verify public leftover loot with a separate donor account.
HEADED=1 SLOWMO=0 bun run verify:kq --base http://localhost:8890 --loot-probe --minutes 15 --level 70
```
...
`--loot-probe` uses four ordinary level-70 fighters and a separate donor. After the first verified kill, the donor drops a dragon chainbody, rune chainbody, amulet of power and 137 rune arrows, then teleports out and logs out. The items become public through the server's normal 100-tick reveal. Its checklist requires public visibility during combat, accepted Take inputs followed by extra ownership of all three valuables, deposits at Shantay, continued eating and combat, no Rune-arrow Take input during a fight, and a confirmed spent-arrow pickup during a continuously observed death-to-respawn wait. The donor's arrow stack may expire before the next wait; ordinary spent arrows provide the recovery evidence. Seeded items are recorded separately and cannot count as natural-drop or soak proof. The donor has 99 HP, Defence and Prayer; production account stats stay unchanged. Use this probe separately from the death and gate-delay probes.
...
Each run prints its directory under `out/e2e/jivekq/<run>/`. Open `report.md` for the checklist, per-trip kills, duration, minimum HP, remaining food and combat screenshots. `observations.jsonl` contains the full scenario sample stream; `cleanup-observations.jsonl` continues recording through escape and logout. Cleanup has a 120-second deadline, bounds browser operations, and reports only observed logouts. Missing pages cannot authorize fixture rescue teleports or erase an earlier verified logout. Unverified logouts fail the run. `proof.json` contains the last 600 observations, trip results, queen search movement and subsequent combat XP, verified Eat/Attack input pairs and consumption times, respawn-to-Attack and later XP-confirmation timings, entry timestamps, rope counts, XP gains, loot movements, minimum HP and the SHA-256 of the served client bundle. Checkpoint screenshots cover readiness, entry, both forms, loot, restocking and recovery. `current.json` and `progress.json` update every ten seconds while the test runs. `out/jivekq-proof.json` points to the latest result. Use each run's own directory when comparing simultaneous harnesses.
