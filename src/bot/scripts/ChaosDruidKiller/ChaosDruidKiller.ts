...
export default class ChaosDruidKiller extends TaskBot {
    ...
    async walkOpening(destination: Tile, radius: number): Promise<boolean> {
        ...
        for (let attempt = 0; attempt < 12; attempt++) {
            ...
            await Traversal.walkTo(destination, { radius,
                timeoutMs: 25_000,
    ...
    private async crossDungeonGates(towardField: boolean): Promise<void> {
        ...
        for (const gate of gates) {
            await Traversal.walkResilient(gate, { radius: 4,
                attempts: 8,
                timeoutMs: 25_000,
                log: message => this.log(`  ${message}`)
            });
    ...
    private async walkExact(dest: Tile): Promise<boolean> {
        ...
        for (let attempt = 0; attempt < 4 && !here(); attempt++) {
            await Traversal.walkResilient(dest, { radius: 0,
                attempts: 6,
                timeoutMs: 25_000,
                log: message => this.log(`  ${message}`)
            });
    ...
    private async yanilleToField(): Promise<boolean> {
        for (let attempt = 0; attempt < 10 && !this.parked; attempt++) {
            ...
            if (zone === 'warrior') {
                ...
                if (await Traversal.walkResilient(this.fieldTile(), { radius: 4,
                    attempts: 8,
                    timeoutMs: 25_000,
                    log: message => this.log(`  ${message}`)
                })) {
            ...
            await Traversal.walkResilient(LEDGE_NORTH_STAND, { radius: 1,
                attempts: 8,
                timeoutMs: 60_000,
                log: message => this.log(`  ${message}`)
            });
    ...
    async goToField(): Promise<boolean> {
        ...
        const arrived = await Traversal.walkResilient(this.fieldTile(), { radius: 4,
            attempts: 8,
            timeoutMs: 25_000,
            log: message => this.log(`  ${message}`)
        });
