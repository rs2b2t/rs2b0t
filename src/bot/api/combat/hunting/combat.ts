...
import { buryOneInFight } from '../fightUpkeep.js';
import { EventSignal } from '../../execution/EventSignal.js';
import { Execution } from '../../execution/Execution.js';
import { Game, facingPlayer } from '../../game/Game.js';
...
export class Fight implements Task {
    ...
    async execute(): Promise<void> {
        ...
        for (let pass = 0; pass < FIGHT_PASSES && performance.now() < deadline; pass++) {
            if (!Game.sceneReady() || EventSignal.pending() || this.host.died || ChatDialog.canContinue() || this.host.shieldReady?.() === false) {
                this.interruptWatch();
                return;
            }
