...
import { driveDialog } from '../../ai/quests/exec/primitives.js';
...
export async function bankRoutine(h: JiveHost, site: DragonSite, opts: BankOpts): Promise<void> {
    ...
    if (!(await withdrawGear(h, site, opts.wear, opts.carry))) {
        await Bank.close();
        return;
    }
...
async function withdrawGear(h: JiveHost, site: DragonSite, wear: readonly string[] = [], carry: readonly string[] = []): Promise<boolean> {
    ...
    if (needsShield(site, h.style())) {
        if (!Equipment.contains(SHIELD) && Inventory.count(SHIELD) === 0 && Bank.count(SHIELD) > 0 && Inventory.isFull()) {
            ...
            if (!(await Execution.delayUntil(() => Bank.isOpen() && Bank.loaded() && Inventory.used() < used, 2500))) {
                return false;
            }
        }
    ...
    return !needsShield(site, h.style()) || Equipment.contains(SHIELD) || Inventory.count(SHIELD) > 0;
}
